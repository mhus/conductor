...
//                    console.flush();
            ...
            TreeNodeList projectsC = out.getArray("projects").orElseGet(() -> out.createArray("projects"));
            ...
            MService.getService(ITreeNodeFactory.class).write(out, new File(file));
            ...
            if (f.exists()) {
                ITreeNode in = MService.getService(ITreeNodeFactory.class).read(f);
                Conductor con = cli.getConductor();
                {
                    ITreeNode p = in.getObject("properties").orElse(null);
                    if (p != null) {
                        ((MProperties) con.getProperties()).putReadProperties(p);
                    }
                }
                TreeNodeList projectsC = in.getArray("projects").orElseGet(() -> in.createArray("projects"));
                for (ITreeNode projectC : projectsC) {

...
REPO_PATH_ZIP="org/summerclouds/conductor-launcher/1.1.2-SNAPSHOT/conductor-launcher-1.1.2-SNAPSHOT-install.zip"
LOCAL_REPO_PATH_ZIP="$HOME/.m2/repository/$REPO_PATH_ZIP"
REMOTE_REPO_PATH_ZIP="https://repo1.maven.org/maven2/$REPO_PATH_ZIP"

# init

if [ ! -d $HOME/.conductor/bin/1.1.2-SNAPSHOT ]; then
  mkdir -p $HOME/.conductor/bin/1.1.2-SNAPSHOT
fi
...
if [ ! -e $LOCAL_REPO_PATH_ZIP ]; then
  if command -v mvn &> /dev/null; then
    mvn org.apache.maven.plugins:maven-dependency-plugin:3.1.2:get \
      -Dartifact=org.summerclouds.conductor:conductor-launcher:1.1.2-SNAPSHOT:zip:install
...
cd $HOME/.conductor/bin/1.1.2-SNAPSHOT
unzip -o $LOCAL_REPO_PATH_ZIP
chmod +x $HOME/.conductor/bin/1.1.2-SNAPSHOT/*.sh

if [ -e $HOME/.conductor/bin/con ]; then
  rm $HOME/.conductor/bin/con
fi
ln -s $HOME/.conductor/bin/1.1.2-SNAPSHOT/con.sh $HOME/.conductor/bin/con
...
echo "Installed 1.1.2-SNAPSHOT in $HOME/.conductor"
